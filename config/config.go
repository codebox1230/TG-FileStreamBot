...
type config struct {
	...
	HashLength     int          `envconfig:"HASH_LENGTH" default:"10"`
...
func Load(log *zap.Logger, cmd *cobra.Command) {
	...
	if ValueOf.HashLength == 0 {
		log.Sugar().Info("HASH_LENGTH can't be 0, defaulting to 10")
		ValueOf.HashLength = 10
	}
	if ValueOf.HashLength > 64 {
		log.Sugar().Info("HASH_LENGTH can't be more than 64, changing to 64")
		ValueOf.HashLength = 64
	}
	if ValueOf.HashLength < 10 {
		log.Sugar().Info("HASH_LENGTH can't be less than 10, defaulting to 10")
		ValueOf.HashLength = 10
	}
